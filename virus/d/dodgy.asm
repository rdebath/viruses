...
;Final version full options Warning!!! Destructive sequence included!
;               ---------------------
...
dxpar equ this word
	...
	mov word ptr ds:[offset temp-2],609Ch	;refac cod de pushf 
pusha
...
;---
	...
	or dh,dh	;head 0?
	jnz contcmp
	cmp cx,000eh	;se redirecteaza 14 si 15 pe 13  presupus cu 
zerouri 
...
contcmp:
	...
	jmp short verificare
hard:
	call callint13;		it was requested a read action for the 
boot
verificare:
	jc giveup
...
normal:
	inc cx		;salvez  cx=0000 cu pusha dupa rep movsw 
=>cx=0001
iar:
...
cautare:
   ...
   mov word ptr cs:[offset temp-2],[(offset peste)-(offset temp)] shl 
8+ 0ebh
